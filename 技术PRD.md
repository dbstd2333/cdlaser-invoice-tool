...
构建面向 Windows 的单机 Electron 应用，在无独立后端服务、无登录的情况下提供可靠的本地数据事务、Excel 导入导出、字段级审计和腾讯云 COS 云备份。
...
- 渲染进程不得直接访问 Node.js、文件系统、SQLite 或 COS 密钥。
...
| 云备份 | 腾讯云 COS Node.js SDK | 腾讯云对象存储 COS |
...
- 管理 BrowserWindow、SQLite、迁移、文件系统、Excel、COS 和系统对话框。
- 承载所有领域服务和事务。
- 校验每个 IPC 请求及发送方。
...
- Region、包含 APPID 的完整 Bucket 名称和对象 Prefix。
- SecretId、SecretKey，临时密钥可附带 SecurityToken。

密钥使用 Electron `safeStorage` 加密，只在主进程内解密，不写入业务数据库、日志或备份。
...
COS 界面由渲染进程的全局设置抽屉承载，建议拆分为：

- `SettingsDrawer`：设置容器和页签切换。
- `CosBackupPanel`：数据加载和操作编排。
- `BackupStatusCard`：连接、最近备份、dirty 状态和错误摘要。
- `CosConnectionForm`：Region、Bucket、Prefix 和 COS 凭据。
...
- 日志按日期滚动，默认保留 14 天。
- 严禁记录客户完整税号、银行账号、COS 密钥、恢复密码和 Excel 全量内容。
