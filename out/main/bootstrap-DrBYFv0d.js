...
var products = (0, drizzle_orm_sqlite_core.sqliteTable)("products", {
	...
	createdAt: (0, drizzle_orm_sqlite_core.text)("created_at").notNull(),
	updatedAt: (0, drizzle_orm_sqlite_core.text)("updated_at").notNull()
}, (table) => ({ nameModelUnique: (0, drizzle_orm_sqlite_core.uniqueIndex)("products_name_model_unique").on(table.nameNormalized, table.modelNormalized) }));
/** 价格版本表 */
var priceVersions = (0, drizzle_orm_sqlite_core.sqliteTable)("price_versions", {
	id: (0, drizzle_orm_sqlite_core.text)("id").primaryKey(),
	productId: (0, drizzle_orm_sqlite_core.text)("product_id").notNull().references(() => products.id),
	...
	productPriceUnique: (0, drizzle_orm_sqlite_core.uniqueIndex)("price_versions_product_price_unique").on(table.productId, table.unitPriceDecimal),
	productIdx: (0, drizzle_orm_sqlite_core.index)("price_versions_product_idx").on(table.productId),
	statusIdx: (0, drizzle_orm_sqlite_core.index)("price_versions_status_idx").on(table.status),
	stockIdx: (0, drizzle_orm_sqlite_core.index)("price_versions_stock_idx").on(table.stockBalance)
}));
/** 销项开票批次表 */
...
var outboundLines = (0, drizzle_orm_sqlite_core.sqliteTable)("outbound_lines", {
	...
	priceVersionId: (0, drizzle_orm_sqlite_core.text)("price_version_id").notNull(),
	name: (0, drizzle_orm_sqlite_core.text)("name").notNull(),
	taxClassificationCode: (0, drizzle_orm_sqlite_core.text)("tax_classification_code").notNull(),
	model: (0, drizzle_orm_sqlite_core.text)("model").notNull(),
	unit: (0, drizzle_orm_sqlite_core.text)("unit").notNull(),
	...
	priceVersionIdx: (0, drizzle_orm_sqlite_core.index)("outbound_lines_price_version_idx").on(table.priceVersionId)
}));
/** 进项批次表 */
...
var inboundLines = (0, drizzle_orm_sqlite_core.sqliteTable)("inbound_lines", {
	...
	priceVersionIdx: (0, drizzle_orm_sqlite_core.index)("inbound_lines_price_version_idx").on(table.priceVersionId)
}));
/** 月底补票导出记录表 */
...
var inventoryLedger = (0, drizzle_orm_sqlite_core.sqliteTable)("inventory_ledger", {
	...
	priceVersionIdx: (0, drizzle_orm_sqlite_core.index)("inventory_ledger_price_version_idx").on(table.priceVersionId, table.createdAt),
...
/** 价格版本新增 */
var priceVersionCreateSchema = zod.z.object({
	productId: zod.z.string().min(1),
	unitPriceDecimal: zod.z.string().min(1)
});
/** 商品价格版本查询 */
...
/** 查询某价格版本的库存流水 */
function queryLedger(priceVersionId, page, pageSize) {
	const raw = getRawDb();
  ...
  `).get(priceVersionId);
    ...
    LIMIT ? OFFSET ?
  `).all(priceVersionId, pageSize, offset).map((r) => ({
			id: r.id,
...
*/
function recomputeBalance(priceVersionId) {
	return getRawDb().prepare(`
    SELECT balance_after FROM inventory_ledger
    WHERE price_version_id = ?
    ORDER BY created_at DESC, id DESC
    LIMIT 1
  `).get(priceVersionId)?.balance_after ?? null;
}
/**
...
function consistencyCheck() {
	const versions = getRawDb().prepare(`
  ...
  `).all();
	...
	for (const v of versions) {
		...
		if (v.stock_balance !== expected) mismatches.push({
			priceVersionId: v.id,
...
/** 联合查询映射为页面二表格行 */
function mapPriceVersionRow(row) {
	return {
		...
		model: row.model,
		unit: row.unit,
		taxClassificationCode: row.tax_classification_code,
		unitPriceDecimal: row.unit_price_decimal,
		taxRate: row.tax_rate,
		stockBalance: row.stock_balance,
		dataStatus: row.data_status,
		productStatus: row.product_status,
		priceVersionStatus: row.price_version_status,
		remark: row.remark,
...
*/
/** 分页查询价格版本（页面二大表格） */
function listPriceVersions(input) {
	const raw = getRawDb();
	const { conditions, params } = buildQueryConditions$1(input);
	const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(" AND ")}` : "";
	const countRow = raw.prepare(`SELECT COUNT(*) as cnt FROM price_versions pv JOIN products p ON p.id = pv.product_id ${whereClause}`).get(...params);
	const offset = (input.page - 1) * input.pageSize;
	return {
		rows: raw.prepare(`
    ...
    LIMIT ? OFFSET ?
  `).all(...params, input.pageSize, offset).map(mapPriceVersionRow),
		total: countRow.cnt,
		page: input.page,
		pageSize: input.pageSize
	};
...
function getPriceVersionsByIds(ids) {
	if (ids.length === 0) return [];
	const raw = getRawDb();
	const placeholders = ids.map(() => "?").join(",");
	return toCamelList(raw.prepare(`SELECT * FROM price_versions WHERE id IN (${placeholders})`).all(...ids)).map(mapPriceVersion);
}
//#endregion
...
function createProduct(input) {
	...
	const dataStatus = taxCode ? "complete" : "incomplete";
	...
	recordAudit({
		...
		entityType: "product",
		entityId: id,
		summary: `新增商品: ${trimInvisible(input.name)}`
	});
	markDirty();
...
function updateProduct(input) {
	...
	db.update(products).set(updates).where((0, drizzle_orm.eq)(products.id, input.id)).run();
...
/** 校验价格版本唯一性 */
function ensurePriceVersionUnique(productId, unitPriceDecimal) {
	if (getRawDb().prepare("SELECT id FROM price_versions WHERE product_id = ? AND unit_price_decimal = ?").get(productId, unitPriceDecimal)) throw new Error("该商品已有相同单价的价格版本");
}
/** 构建商品字段变更记录 */
...
function checkDatabaseConflict(row, nameNorm, modelNorm, unitPriceNormalized, isInitial) {
	...
	const existingProduct = getRawDb().prepare("SELECT id, unit, tax_classification_code FROM products WHERE name_normalized = ? AND model_normalized = ?").get(nameNorm, modelNorm);
	if (existingProduct) {
		...
		if (getRawDb().prepare("SELECT id FROM price_versions WHERE product_id = ? AND unit_price_decimal = ?").get(existingProduct.id, unitPriceNormalized)) errors.push(isInitial ? "该商品和单价已存在" : "该商品和单价已存在，日常导入不允许覆盖");
		else isNewPriceVersion = true;
	} else {
		isNewProduct = true;
...
function confirmInitialImport(token) {
	...
	raw.transaction(() => {
		for (const row of validRows) {
			...
			const pvId = createPriceVersion$1(db, productId, normalizeUnitPrice(row.unitPriceDecimal), row.initialStock ?? 0);
			pvCount++;
			...
			if (initialStock !== 0) {
				...
				if (balanceAfter !== initialStock) db.update(priceVersions).set({
					stockBalance: balanceAfter,
					updatedAt: (/* @__PURE__ */ new Date()).toISOString()
				}).where((0, drizzle_orm.eq)(priceVersions.id, pvId)).run();
			}
			recordAudit({
				action: "catalog.initial_import",
				entityType: "price_version",
				entityId: pvId,
...
function confirmDailyImport(token) {
	...
	raw.transaction(() => {
		for (const row of validRows) {
			const { productId, isNewProduct } = findOrCreateProduct(db, row);
			if (isNewProduct) productCount++;
			const pvId = createPriceVersion$1(db, productId, normalizeUnitPrice(row.unitPriceDecimal), 0);
			pvCount++;
			recordAudit({
...
function findOrCreateProduct(db, row) {
	const nameNorm = normalizeKey(row.name);
	const modelNorm = normalizeKey(row.model);
	const existing = getRawDb().prepare("SELECT id FROM products WHERE name_normalized = ? AND model_normalized = ?").get(nameNorm, modelNorm);
	if (existing) return {
...
/** 创建价格版本 */
function createPriceVersion$1(db, productId, unitPriceDecimal, stockBalance) {
	const pvId = (0, uuid.v7)();
	const now = (/* @__PURE__ */ new Date()).toISOString();
	db.insert(priceVersions).values({
		id: pvId,
		productId,
...
function validateDraft(input) {
	...
	for (const line of input.lines) {
		const pvRow = raw.prepare(`SELECT pv.id, pv.unit_price_decimal, pv.tax_rate, pv.stock_balance, pv.status as pv_status,
                p.name, p.model, p.unit, p.data_status, p.status as p_status
         FROM price_versions pv
         JOIN products p ON p.id = pv.product_id
         WHERE pv.id = ?`).get(line.priceVersionId);
		if (!pvRow) {
...
function registerOutboundIpc() {
	...
	registerHandler(IPC_CHANNELS.outbound.export, outboundExportSchema, async (input, sender) => {
	...
	registerHandler(IPC_CHANNELS.outbound.download, null, async (id, sender) => {
...
function registerReplenishmentIpc() {
	...
	registerHandler(IPC_CHANNELS.replenishment.export, null, async (_input, sender) => {
	...
	registerHandler(IPC_CHANNELS.replenishment.download, null, async (id, sender) => {
...
function matchExistingProduct(name, model, unit, price, quantity) {
	...
	const result = {
		...
		productId: null,
		priceVersionId: null,
		isNewPriceVersion: false,
		matched: false,
		errors: [],
		newProductCount: 0,
		newPriceVersionCount: 0
	};
	const existingProduct = raw.prepare("SELECT id, unit, tax_classification_code, data_status FROM products WHERE name_normalized = ? AND model_normalized = ?").get(nameNorm, modelNorm);
	if (existingProduct) {
		result.productId = existingProduct.id;
		if (existingProduct.unit !== unit) result.errors.push(`单位不一致（已有: ${existingProduct.unit}，导入: ${unit}）`);
		const existingPv = raw.prepare("SELECT id, stock_balance FROM price_versions WHERE product_id = ? AND unit_price_decimal = ?").get(existingProduct.id, price);
		if (existingPv) {
			result.priceVersionId = existingPv.id;
			result.matched = true;
			if (existingPv.stock_balance < 0 && quantity > Math.abs(existingPv.stock_balance)) result.errors.push(`当前库存 ${existingPv.stock_balance}，导入数量 ${quantity} 超过待补数量 ${Math.abs(existingPv.stock_balance)}`);
		} else {
...
function buildInboundPreview(rawRows, fileSha256) {
	...
	for (const [, entry] of aggregatedMap) {
		...
		lines.push({
			...
			priceVersionId: match.priceVersionId,
			isNewPriceVersion: match.isNewPriceVersion,
...
function processInboundLine(db, raw, batchId, batchNo, line) {
	...
	if (!priceVersionId) priceVersionId = createPriceVersion(db, productId, line.unitPriceDecimal);
	...
	db.update(priceVersions).set({
		stockBalance: balanceAfter,
		updatedAt: (/* @__PURE__ */ new Date()).toISOString()
	}).where((0, drizzle_orm.eq)(priceVersions.id, priceVersionId)).run();
}
...
/** 创建价格版本 */
function createPriceVersion(db, productId, unitPriceDecimal) {
	const priceVersionId = (0, uuid.v7)();
	const now = (/* @__PURE__ */ new Date()).toISOString();
	db.insert(priceVersions).values({
		id: priceVersionId,
		productId,
...
/** 读取价格版本当前库存 */
function getStockBalance(raw, priceVersionId) {
	return raw.prepare("SELECT stock_balance FROM price_versions WHERE id = ?").get(priceVersionId)?.stock_balance ?? 0;
}
//#endregion
...
function adjustInventory(input) {
	...
	raw.transaction(() => {
		...
		db.update(priceVersions).set({
			stockBalance: balanceAfter,
			updatedAt: (/* @__PURE__ */ new Date()).toISOString()
		}).where((0, drizzle_orm.eq)(priceVersions.id, input.priceVersionId)).run();
		recordAudit({
