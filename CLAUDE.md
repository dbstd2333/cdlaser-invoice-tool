...
发票库存管理系统 - 面向 Windows 的单机 Electron 桌面应用。
...
- **进程隔离**：主进程承载所有领域服务和事务；预加载通过 contextBridge 暴露最小 API；渲染进程不直接访问 Node.js/SQLite/S3。
...
- S3 密钥使用 Electron safeStorage (DPAPI) 加密存储，不写入业务数据库。
